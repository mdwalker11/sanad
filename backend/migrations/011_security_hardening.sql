-- Sanad security hardening 011
-- Apply after migrations 001-010 in Supabase SQL Editor.
-- This migration closes client-controlled lifecycle, moderation, and money fields.
...
-- A self-submitted worker profile is an application, never an approval.
...
create trigger guard_worker_profile_fields
before insert or update on public.worker_profiles
for each row execute function public.guard_worker_profile_fields();

-- Customer inserts may not forge lifecycle, assignment, or financial fields.
...
begin
  if not public.sanad_is_operator() then
    ...
    new.status := 'awaiting_offers';
    new.selected_worker_id := null;
    new.selected_offer_id := null;
    new.customer_total := null;
  ...
  if new.booking_type in ('scheduled','time_window')
     and (new.preferred_start is null or new.preferred_end is null
          or new.preferred_end <= new.preferred_start) then
...
create trigger guard_customer_order_insert
before insert on public.service_orders
for each row execute function public.guard_customer_order_insert();

-- Offers are accepted only for dispatchable orders and approved workers.
...
begin
  if not public.sanad_is_operator() then
    if new.worker_id <> auth.uid() then
      raise exception 'offer_owner_mismatch';
    end if;
    if not exists (
      select 1 from public.worker_profiles wp
      where wp.user_id = auth.uid() and wp.verification_status = 'approved'
    ) then
      raise exception 'worker_not_approved';
    end if;
    if not exists (
      select 1 from public.service_orders o
      where o.id = new.order_id and o.status = 'awaiting_offers'
    ) then
  ...
  if new.total_amount < 0 or new.visit_amount < 0
     or new.labor_amount < 0 or new.materials_amount < 0
     or new.commission_rate < 0 or new.commission_rate > 100 then
...
create trigger guard_service_offer_insert
before insert on public.service_offers
for each row execute function public.guard_service_offer_insert();

-- Prevent direct client lifecycle updates; approved RPCs remain the transition path.
drop policy if exists "customers can update own orders" on public.service_orders;

-- Useful relationship indexes for dispatch, operator views, and RLS joins.
create index if not exists service_orders_selected_worker_idx
  on public.service_orders(selected_worker_id);
create index if not exists service_orders_address_idx
  on public.service_orders(address_id);
create index if not exists order_media_order_idx
  on public.order_media(order_id);
...
-- Keep private media policy evaluation safe for malformed paths.
